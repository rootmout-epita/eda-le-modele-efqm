...
\begin{figure}
    ...
    \caption{Logo de l'\acrshort{afnor}}
    \label{fig:afnor}
\end{figure}
...
Il s’agit d’une association dont les actions sont visibles pour nous, en tant que consommateurs, au quotidien. C’est elle qui est chargée du contrôle des normes européennes ou françaises avec l'apposition de logos sur les produits qui font notre foyer. Deux d’entre eux sont par ailleurs très répandus, le logo de \acrfull{ce} et le logo de conformité aux \acrfull{nf} sont obligatoires pour les produits souhaitant être disposés à la vente sur l'hexagone. Un objet fabriqué et estampillé de ce label est alors de qualité suffisante pour ne pas mettre en danger les consommateurs.
...
\begin{figure}[!h]
    ...
    \caption{Logos des sous-marques de l'\acrshort{afnor}}%
...
\begin{itemize}[itemsep=0.4cm]
    ...
    \item dans un second temps la méthode \acrshort{radar} est utilisée pour déterminer les actions d'amélioration à mettre en place en priorité, vous devez en obtenir au moins trois~;
    \item ensuite un évaluateur \acrshort{efqm} se déplace sur le site de l’entreprise pour évaluer la  pertinence des actions qui ont été entreprises ainsi que la capacité de l’entreprise à les mettre en place~;
    ...
    \item et finalement l’entreprise obtient un rapport complet ainsi que le diplôme de reconnaissance \acrshort{efqm} délivré par l’\acrshort{afnor}.
\end{itemize}

Une fois que l’entreprise a complètement appliqué le modèle \acrshort{efqm} à ses activités, elle peut prétendre à l’évaluation \gls{gls-r4e}\footnotemark[\value{footnote}] «~Reconnu pour l’Excellence~»
